...
#define TOKEN(NAME, ...) int NAME(source *src){\
	unsigned ch=getch(src); int matched=0;	source old_ctx = *src;\
	__VA_ARGS__;\
}

TOKEN(real, {
	match_repopt( (is_charclass_sys_d(ch)) ); int has_num=matched;	
	match_opt(    (0x2E==(ch)) ); int has_point=matched;	
...
TOKEN(hex, {	//matcher for pattern: "0x%h+"
...
TOKEN(integer, {	//pattern: "%d+"
	match_rep((is_charclass_sys_d(ch)) );
	return -1;
})

TOKEN(id, { //pattern: "%I+%i*"
