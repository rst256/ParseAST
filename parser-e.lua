...
function TypeExprProp:sizeof()
	...
	if self.ArrayDim then size=size*tonumber(tostring(self.ArrayDim)) end
	return size
end
...
UnExpr:insert(
	Seq(
		Alt(lexeme'-', lexeme'!', lexeme'++', lexeme'--', lexeme'*', lexeme'&',
			Seq(lexeme' (', TypeExpr, lexeme' )'):tmpl'($1)'),
	UnExpr):tmpl'$1($2)',
	...
	Seq(kwrd' sizeof', lexeme' (', Alt(TypeExpr, Ident)^'Expr', lexeme' )'):tmpl'(size_t)$Size':hndl(function(tok0, tok, obj)
		if tok~=nil then
		obj.Size=obj.Expr.sizeof or tok0.scope:find(tostring(obj.Expr)).type.sizeof
				end
		return tok, obj
	end)
)
...
Value:add(
	Seq( lexeme' (', Expr^'Cond', lexeme' ?', Expr^'Then', lexeme' :', Expr^'Else', lexeme' )' )
	:tmpl' ($Cond ? $Then : $Else) '
)
...
FuncIdent=LValue
Value:insert(Seq(FuncIdent, lexeme' (', args, lexeme' )'):tmpl'$1($2)')
...
local Label = Seq(Ident, lexeme' :'):tmpl'$1:'
...
Chunk = Alt(
	Goto, Define, Return, Continue, Break, Call
	, Assign,
	Label
)
